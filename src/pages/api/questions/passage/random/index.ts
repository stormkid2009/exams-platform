...
const handler=async(
  ...
  try {   
    ...
    const result = await PassageService.getRandomQuestion({});
  ...
  if(!result.success){
      ...
      // Respond with appropriate error code , message , and detials
  ...
  res.status(200).json({
    ...
    await logApiError(
      "unexpected error in random passage question handler",
